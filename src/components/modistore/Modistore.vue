        ...
        <div class="hero-body">
            <div class="container">
                <div class="columns">
                    <div class="column">
                        <br/>
                        <h1 @click="clearSearch" class="title white-title is-1" style="cursor:pointer">
                            Modistore
                        </h1>
                    </div>
                    ...
                    <div class="column is-one-quarter-desktop is-full-mobile">
                        <div class="modistore-labels-container">
                            <h1 class="modistore-label-title">Catégories</h1>
                                <span v-for="label in labels" :key="label._id" class="button is-medium" @click="selectLabel(label)">{{label.name}}</span>
                        </div>
                    </div>
                    <div class="column is-three-quarter-desktop is-full-mobile">
                        <div class="columns is-multiline is-mobile">
                            <div v-for="app in (searchContent.length ? applicationsFiltered : applications)" :key="app._id" class="column is-one-third-widescreen is-half-desktop is-full-mobile is-full-tablet"> 
...
export default {
    ...
    methods: {
        ...
        async selectLabel (label) {
            ...
            const resp= await ModistoreService.getAllAppsByLabel(this.$cookies.get('api_token'), label.name);
            if (resp.data.sucess) {
        ...
        async getLabels () {
            const resp = await ModistoreService.getAllLabels(this.$cookies.get('api_token'));
            if (resp.data.sucess) {
                this.labels = resp.data.labels;
            }
            else
            {
                this.$parent.notification = {type: 'failure', message: 'Erreur lors de la récupération des labels.'}
            }
        },
        ...
        search: async function(search) {
            if (search.length) {
                let resp = await ModistoreService.getAppsBySearchName(this.$cookies.get("api_token"), search);
                if (resp.data.sucess) {
                    let apps = resp.data.applications;
                    apps.forEach(app => {
                        if (this.current_user.application_list.findIndex(a => a === app._id) != -1)
