...
module.exports = function(app, apiRoutes) {
    ...
    apiRoutes.patch('/infos/info', function(req, res) {
        if (!req.body._id|| !req.body.title || !req.body.content)
            return res.json({success: false, message: 'Error: request incomplete'});
        User.findOne({
            token: req.headers['x-access-token'],
        }, function (err, user) {
            ...
            if (!user)
                ...
                Info.findOne({
                    ...
                    if (!info)
                        ...
                        Info.update({
                            _id: info.id}, {content:req.body.content, title:req.body.title, updated_at:new Date()}, function(err) {
                            if (!err) {
                                return res.json({success: true, message: 'Info update success'})
                            }
                            else
                                return res.json({success: false, message: 'Info update Failed'})
                            });
                        }
                    else
